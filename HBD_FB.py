...
# データファイルの名前を作る（ID_日付）
file_name = exp_date + subj_id + "_fb"
file_name_csv = os.path.join("data/exp/" + file_name + ".csv")
...
wrong_text = visual.TextStim(win, text = "incorrect")
feedback_length = 1
...
for T in HBDList:
    ...
    with open(file_name_csv, "a", encoding = "cp932") as f:
        writer = csv.writer(f, lineterminator = "\n")
        writer.writerow([
            subj_id, trial_index, delay, rt, key, ans
            ])
    # ログファイルを保存
    logging.flush()
